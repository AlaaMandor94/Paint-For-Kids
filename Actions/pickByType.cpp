...
void pickByType::Execute() {
	...
	for (int i = 0; i < pManager->getFigCount(); i++) {
		...
		if (Fig->getFigureName() == "SQUARE") {
			...
			shapes[2]++;	// indx 1 represents no. Ellipse
		}
		else {
			shapes[3]++;	// indx 2 represents no. Hexagons
		}
	}//end of for loop 
...
// count how many categories we have  
	...
	if (no_shapes > 1) {
		...
		if (Fig->getFigureName() == "SQUARE") {
			...
			pGUI->PrintMessage("Collect all TRIANGLE shapes");
		}
		else {
			no_shapesEachCategory = shapes[3];
		...
		while (no_shapesEachCategory > 0) {
			...
			if (p.y > UI.ToolBarHeight || p.x > (UI.MenuItemWidth * PLAY_ITM_COUNT)) {
				...
				if (selectedShape != NULL) {
					...
					else if (Fig->getFigureName() == "TRIANGLE" && (selectedShape->getFigureName() == "TRIANGLE")) {
						score(1);
						selectedShape->HideShape();
						pManager->UpdateInterface();
						no_shapesEachCategory--;
